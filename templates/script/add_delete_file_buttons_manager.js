const urlOrigin = window.location.href.split("manager")[0];
var recommId = window.location.pathname.match(/\d+/g);
...
function createDeleteFileButton(file_type, row_css_id) {
  ...
  if (trackChangeFileLink && recommId[0]) {
    recommId = recommId[0];
    ...
    if (trackChangeFileSpan) {
      deleteUrl =
        urlOrigin +
        "manager_actions/delete_recommendation_file?recommId=" +
        recommId +
        "&fileType=" +
        file_type;
