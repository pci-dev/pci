...
def add_new_language():
    ...
    if not translation_value:
        session.flash = f"No translation for {translated_field.value} in {lang.value.english_name}"
        return

    return _generate_lang_form(translation_value, is_textarea)
...
def _generate_form(article: Article, translated_field: TranslatedFieldType, english_field: str):
    ...
    form.append(_generate_lang_selector(article, translated_field))
    form.append(HR())

    lang_form: List[FORM] = []
    if translations_value:
        for translation_value in translations_value:
            lang_form.append(_generate_lang_form(translation_value, is_textarea))
    form.append(DIV(*lang_form, _id="lang-form-list"))

    return DIV(*form)


def _generate_lang_selector(article: Article, translated_field: TranslatedFieldType):
    ...
    url = cast(str,
               URL(c="translations",
                   f="add_new_language",
                   vars=dict(article_id=article.id, field=translated_field.value),
                   user_signature=True,
                   host=host,
                   scheme=scheme,
                   port=port)
    ...
    return FORM(
        DIV(
            ...
            BUTTON("Add new translation", _id="add-translation", _class="btn btn-default", _link=url),
            _class="col-sm-9"
        ),
         _class="form-group"
    )
...
def _generate_lang_form(translation_value: TranslatedFieldDict, is_textarea: bool):
    ...
    return FORM(
                ...
                DIV(
                    input,
                    A("Save", _class="btn btn-default"),
                    A("Delete", _class="btn btn-danger"),
                    _class="col-sm-9"
                ),
                _class="form-group", _style="margin-bottom: 10px"
            )
