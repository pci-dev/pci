...
if request.function == "manage":
    ...
    if manager_action is None and request.args(0) == "auth":
        manager_action = dict(role=auth.settings.auth_manager_role, heading=T("Manage Access Control"), tables=[auth.table_user(), auth.table_group(), auth.table_permission()])
    manager_role = manager_action.get("role", None) if manager_action else None
    if not (gluon.fileutils.check_credentials(request) or auth.has_membership(manager_role)):
        raise HTTP(403, "Not authorized")
    menu = False
elif (request.application == "admin" and not session.authorized) or (request.application != "admin" and not gluon.fileutils.check_credentials(request)):
...
def select():
    ...
    form = FORM(
        TABLE(
            TR(T("Query:"), "", INPUT(_style="width:400px", _name="query", _value=request.vars.query or "", requires=IS_NOT_EMPTY(error_message=T("Cannot be empty")))),
            TR(
                T("Update:"),
    ...
    if form.accepts(request.vars, formname=None):
        ...
        try:
            ...
            if form.vars.update_check and form.vars.update_fields:
                db(query, ignore_common_filters=True).update(**eval_in_global_env("dict(%s)" % form.vars.update_fields))
            ...
            if is_imap:
                fields = [db[table][name] for name in ("id", "uid", "created", "to", "sender", "subject")]
            if orderby:
                rows = db(query, ignore_common_filters=True).select(*fields, limitby=(start, stop), orderby=eval_in_global_env(orderby))
            else:
                rows = db(query, ignore_common_filters=True).select(*fields, limitby=(start, stop))
    ...
    return dict(form=form, table=table, start=start, stop=stop, step=step, nrows=nrows, rows=rows, query=request.vars.query, formcsv=formcsv, tb=tb)
...
def table_template(table):
    ...
    rows.append(TR(TD(FONT(table, _face=face_bold, _color=bgcolor), _colspan=3, _cellpadding=cellpadding, _align="center", _bgcolor=color)))
    for row in db[table]:
        rows.append(
            TR(
                TD(FONT(row.name, _color=color, _face=face_bold), _align="left", _cellpadding=cellpadding, _border=border),
                TD(FONT(row.type, _color=color, _face=face), _align="left", _cellpadding=cellpadding, _border=border),
                TD(FONT(types(row), _color=color, _face=face), _align="center", _cellpadding=cellpadding, _border=border),
            )
        )
    return "< %s >" % TABLE(*rows, **dict(_bgcolor=bgcolor, _border=1, _cellborder=0, _cellspacing=0)).xml()
...
def bg_graph_model():
    ...
    for tablename in db.tables:
        ...
        graph.add_node(tablename, name=tablename, shape="plaintext", label=table_template(tablename))

    for n, key in enumerate(subgraphs.iterkeys()):
        graph.subgraph(nbunch=subgraphs[key]["tables"], name="cluster%d" % n, style="filled", color=subgraphs[key]["meta"]["color"], label=subgraphs[key]["meta"]["group"])
...
def manage():
    ...
    if request.args(0) == "auth":
        ...
        if table == auth.table_user():
            ...
            orderby = "group_id"
    kwargs = dict(user_signature=True, maxtextlength=1000, orderby=orderby, linked_tables=linked_tables)
