...
def my_awaiting_articles():
    ...
    db.t_articles.user_id.represent = lambda userId, row: common_small_html.mkAnonymousArticleField(auth, db, row.anonymous_submission, row.article_submitter.replace('<span>', '').split('</span>')[0] if row.article_submitter else common_small_html.mkUser(auth, db, userId))
    ...
    if parallelSubmissionAllowed:
        ...
        fields = [
            ...
            db.t_articles.user_id,
            db.t_articles.auto_nb_recommendations,
            db.t_articles.article_submitter,
        ]
