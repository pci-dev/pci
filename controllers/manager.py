...
def fetch_query(action, recommenderId):
    ...
    queries = {
        ...
        "late_reviews": get_recommendation_DAL_query(f"SELECT DISTINCT recomm.id FROM t_articles a , t_reviews trev, t_recommendations recomm, v_article_recommender v_art WHERE recomm.article_id = a.id and trev.recommendation_id = recomm.id and recomm.recommender_id={recommenderId} and recomm.id=v_art.recommendation_id and trev.acceptation_timestamp + convert_duration_to_sql_interval(trev.review_duration) < NOW()"),
    }
