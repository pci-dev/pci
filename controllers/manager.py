...
def fetch_query(action, recommenderId):
    ...
    queries = {
        ...
        "requiring_action" : get_recommendation_DAL_query(f"SELECT DISTINCT * FROM (\
                                                                   SELECT DISTINCT recomm.* AS nb FROM t_articles art JOIN t_recommendations recomm ON art.id = recomm.article_id JOIN v_article_recommender v_art ON recomm.id = v_art.recommendation_id LEFT JOIN (SELECT recommendation_id, COUNT(*) AS num_reviews FROM t_reviews GROUP BY recommendation_id\
                                                                    ) rev ON recomm.id = rev.recommendation_id WHERE art.status = 'Under consideration' AND recomm.recommender_id = {recommenderId} GROUP BY  recomm.id, art.report_stage, rev.num_reviews HAVING ((art.report_stage = 'STAGE 1' AND COALESCE(rev.num_reviews, 0) < 2) OR (art.report_stage = 'STAGE 2' AND COALESCE(rev.num_reviews, 0) = 0)) \
                                                            UNION ALL\
        ...
        "requiring_reviewers": get_recommendation_DAL_query(f"SELECT DISTINCT recomm.id FROM t_articles art JOIN t_recommendations recomm ON art.id = recomm.article_id JOIN v_article_recommender v_art ON recomm.id = v_art.recommendation_id LEFT JOIN (SELECT recommendation_id, COUNT(*) AS num_reviews FROM t_reviews GROUP BY recommendation_id) rev ON recomm.id = rev.recommendation_id WHERE art.status = 'Under consideration'\
                                                                AND recomm.recommender_id = {recommenderId} GROUP BY recomm.id, art.report_stage, rev.num_reviews HAVING ((art.report_stage = 'STAGE 1' AND COALESCE(rev.num_reviews, 0) < 2) OR (art.report_stage = 'STAGE 2' AND COALESCE(rev.num_reviews, 0) = 0))"),
