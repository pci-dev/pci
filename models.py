...
# # -------------------------------------------------------------------------
appName = global_config.get("app", "name")
scheme = global_config.get("alerts", "scheme")
host = global_config.get("alerts", "host")
port = common_tools.takePort(global_config.get("alerts", "port"))
pdf_max_size = global_config.getint("config", "pdf_max_size", fallback=10)

scheduledSubmissionActivated = global_config.getboolean("config", "scheduled_submissions", fallback=False)
pciRRactivated = global_config.getboolean("config", "registered_reports", fallback=False)

postprint = global_config.getboolean("config", "postprint", fallback=False)
...
db = DAL(
    global_config.get("db", "uri"),
    pool_size=global_config.get("db", "pool_size"),
    migrate_enabled=global_config.get("db", "migrate"),
    check_reserved=["all"],
)
...
cfg.description = global_config.get("app", "description")

db.cfg = cfg
db.conf = global_config
# -------------------------------------------------------------------------
...
auth = Auth(session,
            db,
            extra_fields=auth_extra_fields,
            use_username=False,
...
# auth.messages.verify_email_subject = "%s: validate your registration" % global_config.get("app", "longname")
# auth.messages.verify_email = (
#     """
# Welcome %(username)s!

# To complete your registration with the """
#     + global_config.get("app", "longname")
...
#     + global_config.get("app", "thematics")
