...
from py4web.utils.form import CheckboxWidget, RadioWidget, TextareaWidget
from pydal import migrator
from pydal.tools.tags import Tags

from .modules.app_modules.country import Country
from .modules.app_components.custom_field import RequiredField
from .modules.models.membership import Membership
from .modules.models.article import Article
from .modules.models.recommendation import Recommendation
from .modules.models.user import User
from .modules.models.mail_queue import MailQueue
from .modules.models.group import Role
from .modules.models.review import ReviewDuration, Review, ReviewState
from .modules.app_modules.common_small_html import mkDOI
from py4web import DAL, HTTP, redirect, request, response
from py4web.core import Fixture, Session
from py4web.utils.auth import Auth
from pydal.objects import Field
from yatl import A, DIV, I, SPAN, XML
from .settings import APP_FOLDER
from .common import db, config, session, T, is_rr, host, auth, postprint, appName, scheme, port, pdf_max_size, coar
from .modules.app_modules.orcid import ORCID_NUMBER_FIELD_TYPE, ORCID_NUMBER_LENGTH_WITH_HYPHEN, OrcidTools, OrcidValidator
from .modules.app_modules.common_tools import URL, BR, B
...
from .modules.app_components.custom_validator import CUSTOM_VALID_URL, VALID_LIST_NAMES_MAIL, VALID_DOI, TEXT_CLEANER, VALID_TEMPLATE
...
from pydal.validators import IS_DATE, IS_FILE, IS_IN_DB, IS_IN_SET, IS_EMPTY_OR, IS_INT_IN_RANGE, IS_LENGTH, IS_LIST_OF, IS_LIST_OF_EMAILS, IS_NOT_EMPTY, IS_URL, Validator, IS_GENERIC_URL, IS_HTTP_URL
...
from .modules.app_modules import common_tools
...
cfg = cast(Any, db.config[1])

cfg.host = host.split(".")[0]
cfg.description = config.get("app", "description")
...
allowed_review_filetypes = "pdf" if not is_rr else allowed_upload_filetypes
...
# # -------------------------------------------------------------------------
# response.formstyle = global_config.get("forms", "formstyle")  # or 'bootstrap3_stacked' or 'bootstrap2' or other
# response.form_label_separator = global_config.get("forms", "separator") or ""
...
auth.extra_auth_user_fields = [
    ...
    Field("email_options", type="list:string", label=SPAN(T("Opt in/out of being cc'ed")),
          ...
          widget=CheckboxWidget, default=list(email_options)
    ),
    ...
    Field(
        ...
        type="string",
        label=DIV(
            SPAN(T("Do you wish to receive the PCI newsletter")),
    ...
    Field("registration_datetime", type="datetime", default=datetime.today(), label=T("Registration date & time"), writable=False, readable=False),
...
auth_group = db.define_table(
    "auth_group",
    Field("id", type="id"),
    Field("role", required=True, notnull=True),
    Field("description"),
)

auth_membership = db.define_table(
    "auth_membership",
    Field("id", type="id"),
    Field("group_id", type="reference auth_group", ondelete="RESTRICT"),
    Field("user_id", type="reference auth_user", ondelete="RESTRICT"),
    format="%(keyword)s",
    singular="User role",
    plural="User roles"
)

auth.has_membership = Membership.has_membership
...
db.auth_user.action_token.label = T("Registration key")
db.auth_user.action_token.writable = db.auth_user.action_token.readable = auth.has_membership(role=Role.ADMINISTRATOR)
db.auth_user.action_token.requires = IS_IN_SET(("", "blocked"))
...
# # db.auth_user.email.requires=IS_NOT_EMPTY()
# db.auth_user.action_token.label = T("Registration key")
# db.auth_user.action_token.writable = db.auth_user.action_token.readable = auth.has_membership(role="administrator")
# db.auth_user.action_token.requires = IS_IN_SET(("", "blocked"))
# db.auth_user._format = "%(last_name)s, %(first_name)s"
# db.auth_group._format = "%(role)s"
# db.auth_user.password.label = SPAN(T("Password")) + SPAN(" * ", _style="color:red;")
...
def send_two_factor_email(user, code):
    try:
        auth.sender.send(
            to=[user.email],
            subject= "%s: validate your registration" % config.get("app", "longname"),
            body=f"You're verification code is {code}",
            sender=config.get("contact", "contact"),
        )
    except Exception as e:
        print(e)
    return code

auth.param.two_factor_required = True
...
def newRegistration(s, f):
    o = s.select().first()
    # handle missing "action_token" on password reset and account not confirmed
    if "action_token" not in f.keys():
        f["action_token"] = ""
    if o.action_token != "" and f["action_token"] == "" and (o["recover_email_key"] is None or o["recover_email_key"] == ""):
...
def is_spam(user):
    chars = config.get("config", "banned_chars")
    return (
...
def discard_spam(user: User, db):
    from .modules.app_modules.common_tools import delete_user_from_PCI
...
def ondelete_user(set: ...):
    ...
    if user_to_delete:
        delete_user_from_PCI(user_to_delete)
...
appContactLink=A(config.get("contacts", "managers"), _href="mailto:" + config.get("contacts", "managers"))
parallelSubmissionAllowed = config.get("config", "parallel_submission", fallback=False)
...
db.define_table(
    ...
    Field("anonymous_submission", type="boolean", default=False, label=(
        T("I wish an anonymous submission (submitter concealed from reviewers)") if is_rr else
    ...
    RequiredField("doi", type="string", label=T("Most recent DOI (or URL)"), length=512, unique=False, default="https://", represent=lambda text, row: mkDOI(text),
        requires=VALID_DOI() if not is_rr else CUSTOM_VALID_URL(),
        comment=SPAN(T("Note: for Stage 1 submissions, please make sure the link points exclusively to the manuscript file (and not to the broader project folder), and that any other links to supplementary materials, appendices, data, code, etc. are all within the manuscript file") if is_rr else "")),
    ...
    RequiredField("ms_version", type="string", length=1024, label=SPAN(T("Most recent version of the manuscript"), T(' (e.g. 1)')), default="",
        requires=IS_INT_IN_RANGE(1, 101) if not is_rr else None),
    Field("picture_rights_ok",  widget=widget(_type="hidden") if not is_rr else "",type="boolean", label=T("Picture right")),
    RequiredField("uploaded_picture", type="upload", label=T("Picture"),
        ...
        requires=IS_NOT_EMPTY(error_message=T("Please upload a picture"))) if not is_rr else
    ...
    Field("results_based_on_data", type="string", label="", requires=IS_IN_SET(db.data_choices), widget=RadioWidget,),
    ...
    Field("scripts_used_for_result", type="string", label="", requires=IS_IN_SET(db.script_choices), widget=RadioWidget,),
    ...
    Field("codes_used_in_study", type="string", label="", requires=IS_IN_SET(db.code_choices), widget=RadioWidget,),
    ...
    Field("upload_timestamp", type="datetime", default=datetime.today(), label=T("Submission date")),
    ...
    Field("last_status_change", type="datetime", default=datetime.today(), label=T("Last status change")),
    ...
    Field("doi_of_published_article", type="string", label=T("DOI of published article"), length=512, unique=False, represent=lambda text, row: mkDOI(text), requires=IS_EMPTY_OR(IS_URL(mode='generic',allowed_schemes=['http', 'https'],prepend_scheme='https')), comment=T("URL must start with http:// or https://")),
    Field(
        "cover_letter",
        type="text",
        length=2097152,
        label=T("Cover letter (for the initial submission only, not for resubmissions)") if not is_rr else T("Cover letter"),
        writable=False,
        readable=False,
...
db.define_table(
    ...
    Field("recommendation_timestamp", type="datetime", default=datetime.today(), label=T("Recommendation start"), writable=False, requires=IS_NOT_EMPTY()),
    Field("validation_timestamp", type="datetime", label=T("Validation date")),
    Field("last_change", type="datetime", default=datetime.today(), label=T("Last change"), writable=False),
...
db.review_duration_requires = IS_IN_SET(db.review_duration_choices
        + ((db.review_duration_scheduled_track,) if is_rr else ())
)
...
db.define_table(
    ...
    Field(
        ...
        label=T("AND/OR Upload review as PDF") if not is_rr else (
...
from .modules.app_modules.emailing import isScheduledTrack
...
db.define_table(
    ...
    Field("comment_datetime", type="datetime", default=datetime.today(), label=T("Date & time"), writable=False),
    migrate=False,
    singular=T("Comment"),
    plural=T("Comments"),
...
db.define_table(
    ...
    Field("sending_date", type="datetime", label=T("Sending date"), default=datetime.today()),
...
db.define_table(
    ...
    Field(
        ...
        label=SPAN(
            ...
            A("Section 2.16 of the Guide to Authors. ", _target="_blank"),
            T(
    ...
    Field("tracked_changes_url",
        ...
        length=512, unique=False, represent=lambda text, row: mkDOI(text), comment=T("URL must start with http:// or https://")
    ),
    Field(
        "q25",
        type="boolean",
