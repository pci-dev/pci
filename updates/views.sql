...
SELECT 
    ...
    (SELECT COUNT(DISTINCT id) FROM (
        SELECT DISTINCT recomm.id FROM t_articles a, t_reviews trev, t_recommendations recomm, v_article_recommender v_art WHERE recomm.article_id = a.id and trev.recommendation_id = recomm.id and a.status = 'Under consideration' and recomm.recommender_id=recommender.id and recomm.id=v_art.recommendation_id GROUP BY recomm.id, a.id, recomm.recommender_id HAVING (COUNT(trev.id) < 2 and a.report_stage = 'STAGE 1') or (COUNT(trev.id) = 0 and a.report_stage = 'STAGE 2')
		    UNION ALL
	    SELECT DISTINCT recomm.id FROM t_articles a, t_reviews trev, t_recommendations recomm, v_article_recommender v_art WHERE recomm.article_id = a.id and trev.recommendation_id = recomm.id and a.status in ('Under consideration', 'Awaiting revision') and recomm.recommender_id=recommender.id and recomm.id=v_art.recommendation_id and trev.review_state = 'Review completed' GROUP BY recomm.id, a.id, recomm.recommender_id HAVING COUNT(trev.id) >= 2) requiring_action_nb) AS requiring_action,
    (SELECT COALESCE(SUM(nb), 0) FROM ((SELECT COUNT(*) AS nb FROM t_articles a , t_reviews trev, t_recommendations recomm, v_article_recommender v_art WHERE recomm.article_id = a.id and trev.recommendation_id = recomm.id  and a.status = 'Under consideration' and recomm.recommender_id=recommender.id and recomm.id=v_art.recommendation_id GROUP BY a.id, recomm.recommender_id HAVING (COUNT(trev.id) < 2 and a.report_stage = 'STAGE 1') or (COUNT(trev.id) = 0 and a.report_stage = 'STAGE 2'))) subquery) AS requiring_reviewers,
    (SELECT COALESCE(SUM(nb), 0) FROM ((SELECT COUNT(DISTINCT recomm.id) AS nb FROM t_articles a , t_reviews trev, t_recommendations recomm, v_article_recommender v_art WHERE recomm.article_id = a.id and trev.recommendation_id = recomm.id  and  a.status in ('Under consideration', 'Awaiting revision') and recomm.recommender_id=recommender.id and recomm.id=v_art.recommendation_id and trev.review_state = 'Review completed' GROUP BY recomm.id, a.id, recomm.recommender_id HAVING COUNT(trev.id) >= 2)) subquery) AS required_reviews_completed,
