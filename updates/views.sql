...
SELECT 
    ...
    (SELECT COUNT(DISTINCT id) FROM (
        SELECT DISTINCT recomm.id FROM t_articles a , t_reviews trev, t_recommendations recomm, t_suggested_recommenders ts WHERE ts.article_id = a.id and trev.recommendation_id = recomm.id and recomm.article_id = a.id and a.status = 'Under consideration' and ts.suggested_recommender_id=recommender.id GROUP BY recomm.id, a.id, ts.suggested_recommender_id HAVING (COUNT(trev.id) < 2 and a.report_stage = 'STAGE 1') or (COUNT(trev.id) = 0 and a.report_stage = 'STAGE 2')
		    UNION ALL
	    SELECT DISTINCT recomm.id FROM t_articles a , t_reviews trev, t_recommendations recomm, t_suggested_recommenders ts WHERE ts.article_id = a.id and trev.recommendation_id = recomm.id and recomm.article_id = a.id and ts.suggested_recommender_id=recommender.id GROUP BY recomm.id, a.id, ts.suggested_recommender_id HAVING COUNT(trev.id) filter (WHERE trev.acceptation_timestamp is not null) = COUNT(trev.id) filter (WHERE trev.review_state = 'Review completed')
    ...
    (SELECT COALESCE(SUM(nb), 0) FROM ((SELECT COUNT(DISTINCT recomm.id) AS nb FROM t_articles a , t_reviews trev, t_recommendations recomm, t_suggested_recommenders ts WHERE ts.article_id = a.id and trev.recommendation_id = recomm.id and recomm.article_id = a.id and  a.status in ('Under consideration', 'Awaiting revision') and ts.suggested_recommender_id=recommender.id GROUP BY recomm.id, a.id, ts.suggested_recommender_id HAVING COUNT(trev.id) filter (WHERE trev.acceptation_timestamp is not null) = COUNT(trev.id) filter (WHERE trev.review_state = 'Review completed'))) subquery) AS required_reviews_completed,
    (SELECT COUNT(DISTINCT recomm.id) FROM t_articles a , t_reviews trev, t_recommendations recomm, t_suggested_recommenders ts WHERE ts.article_id = a.id and trev.recommendation_id = recomm.id and recomm.article_id = a.id and ts.suggested_recommender_id=recommender.id and trev.acceptation_timestamp + convert_duration_to_sql_interval(trev.review_duration) > NOW()) AS late_reviews
FROM 
