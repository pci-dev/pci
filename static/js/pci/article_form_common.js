...
async function saveForm(e) {
    ...
    values.sugg_reviewers.push(...Array.from(sugg_reviewers_inputs || [], input => input.value));
    
    const opp_reviewers_inputs = opp_reviewers_ul?.getElementsByTagName('input');
    values.opp_reviewers.push(...Array.from(opp_reviewers_inputs || [], input => input.value));
...
function addInnerHTMLInnerForListValue(inputId, values) {
        ...
        for (const value of values) {
            ...
            <li>
                <div class="input-group" style="width: 100%">
                    <input class="string form-control" id="${inputId}" name="data_doi" type="text" value="${value}">
                </div>
            </li>`;
        }
    }
...
function loadFormSaved() {
    if (typeof savedListStr !== 'undefined' && savedListStr != null) {
        addInnerHTMLInnerForListValue('t_articles_data_doi', savedListStr.data_doi);
        addInnerHTMLInnerForListValue('t_articles_scripts_doi', savedListStr.scripts_doi);
        addInnerHTMLInnerForListValue('t_articles_codes_doi', savedListStr.codes_doi);
        addInnerHTMLInnerForListValue('t_articles_suggest_reviewers', savedListStr.sugg_reviewers);
        addInnerHTMLInnerForListValue('t_articles_competitors', savedListStr.opp_reviewers);
    }
