...
######################################################################################################################################################################
def get_mail_template_hash(db, hashTag, myLanguage="default"):
    query = (db.help_texts.hashtag == hashTag) & (db.help_texts.lang == myLanguage)
...
def mkFooter():
    # init template_vars with common infos 
    template_vars = get_app_config()
    
    # add vars to template_vars
    baseurl = URL(c="default", f="index", scheme=template_vars["scheme"], host=template_vars["host"], port=template_vars["port"])
    profileurl = URL(c="default", f="user", args=("login"), vars=dict(_next=URL(c="default", f="user", args=("profile"))), scheme=template_vars["scheme"], host=template_vars["host"], port=template_vars["port"])
    template_vars["baseurl"] = baseurl
    template_vars["profileurl"] = profileurl
    
    return XML(get_mail_template("mail_footer.html") % template_vars)
    ...
    if article and article.user_id is not None:
        ...
        if article.status == "Pending" and newStatus == "Awaiting consideration":
            ...
            if article.parallel_submission:
                content = get_mail_template("submitter_parallel_preprint_submitted.html") % locals()
            else:
                parallelText = ""
                ...
                content = get_mail_template("submitter_preprint_submitted.html") % locals()
            ...
            if article.parallel_submission:
                content = get_mail_template("submitter_parallel_preprint_under_consideration.html") % locals()

            else:
                content = get_mail_template("submitter_preprint_under_consideration.html") % locals()
            ...
            content = get_mail_template("submitter_cancelled_submission.html") % locals()
            # (gab) SAME CONTENT :
            # if article.parallel_submission:
            # else:
            ...
            content = get_mail_template("submitter_rejected_submission.html") % locals()
            ...
            recommTarget = URL(c="user", f="recommendations", vars=dict(articleId=articleId), scheme=scheme, host=host, port=port)
            recommendation = old_common.mkFeaturedArticle(auth, db, article, printable=True, scheme=scheme, host=host, port=port)
            content = get_mail_template("submitter_not_considered_submission.html") % locals()
            ...
            recommendation = old_common.mkFeaturedArticle(auth, db, article, printable=True, scheme=scheme, host=host, port=port)
            content = get_mail_template("submitter_awaiting_submission.html") % locals()
            ...
            content = get_mail_template("submitter_recommended_preprint.html") % locals()
            ...
            linkTarget = XML(URL(c="user", f="my_articles", scheme=scheme, host=host, port=port))
            content = get_mail_template("submitter_preprint_status_changed.html") % locals()
    ...
    if article is not None:
        ...
        for myRecomm0 in db(db.t_recommendations.article_id == articleId).select(db.t_recommendations.recommender_id, distinct=True):
            ...
            try:
                myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                if len(attach) > 0:
                    mail_resu = mail.send(to=[destAddress], subject=mySubject, message=myMessage, attachments=attach,)
    ...
    if recomm:
        ...
        if article:
            ...
            if theUser:
                ...
                content = get_mail_template("reviewer_review_completed.html") % locals()
...
def do_send_email_to_recommender_co_recommender_considerated(session, auth, db, pressId):
    ...
    if recomm:
        ...
        if article:
            ...
            content = get_mail_template("recommender_co_recommender_considerated.html") % locals()
            try:
                myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                mail_resu = mail.send(to=[destAddress], subject=mySubject, message=myMessage,)
            except:
...
def do_send_email_to_recommenders_press_review_declined(session, auth, db, pressId):
    ...
    if recomm:
        ...
        if article:
            ...
            content = get_mail_template("recommender_co_recommender_declined.html") % locals()
...
def do_send_email_to_recommenders_press_review_agreement(session, auth, db, pressId):
    ...
    if recomm:
        ...
        if article:
            ...
            content = get_mail_template("recommender_co_recommender_agreement.html") % locals()
...
def do_send_email_to_recommenders_review_considered(session, auth, db, reviewId):
    ...
    if recomm:
        ...
        if article:
            ...
            content = get_mail_template("recommender_review_considered.html") % locals()
...
def do_send_email_to_recommenders_review_declined(session, auth, db, reviewId):
    ...
    if recomm:
        ...
        if article:
            ...
            content = get_mail_template("recommender_review_declined.html") % locals()
            ...
            if recomm:
                if recomm.recommender_id != rev.reviewer_id:
                    ...
                    if article:
                        ...
                        content = get_mail_template("reviewer_review_invitation.html") % locals()
        ...
        if lastRecomm:
            ...
            for rev in reviewers:
                if rev is not None and rev.reviewer_id is not None:
                    ...
                    content = get_mail_template("reviewers_article_cancellation.html") % locals()
...
def do_send_mail_admin_new_user(session, auth, db, userId):
    ...
    if user:
        ...
        if len(dest) > 0:  # TODO: also check elsewhere
            try:
                myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                mail_resu = mail.send(to=dest, subject=mySubject, message=myMessage,)
            except:
                pass
    if mail_resu:
...
def do_send_mail_new_membreship(session, auth, db, membershipId):
    ...
    if user and group:
        ...
        if group.role == "recommender":
            ...
            try:
                myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                mail_resu = mail.send(to=[user["email"]], subject=mySubject, message=myMessage,)
                # time.sleep(mail_sleep)
            except:
                pass
            if mail_resu:
...
def do_send_email_to_managers(session, auth, db, articleId, newStatus):
    ...
    if article:
        ...
        for manager in managers:
            try:
                mail_resu = mail.send(to=[manager.email], subject=mySubject, message=myMessage,)
            except Exception as e:
                raise (e)
            if mail_resu:
    ...
    if recomm:
        ...
        if article:
            ...
            if theUser:
                ...
                try:
                    myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                    mail_resu = mail.send(to=[theUser["email"]], subject=mySubject, message=myMessage,)
                except:
                    pass
                if mail_resu:
                    report.append("email sent to %s" % destPerson.flatten())
    ...
    if recomm:
        ...
        if article:
            ...
            for rev in reviewers:
                ...
                try:
                    myMessage = render(filename=mail_layout, context=dict(content=XML(content), footer=mkFooter()))
                    mail_resu = mail.send(to=[rev.auth_user.email], subject=mySubject, message=myMessage,)
                    if review.emailing:
...
def do_send_email_to_reset_password(session, auth, db, userId):
    ...
    linkTarget = URL(
        c="default", f="user", args=["reset_password"], scheme=scheme, host=host, port=port, vars=dict(key=fkey)
