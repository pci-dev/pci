...
def mkSuggestedRecommendersButton(auth, db, row):
    if row.status == "Pending" or row.status == "Awaiting consideration":
        return A(
            XML((db.v_suggested_recommenders[row.id]).suggested_recommenders.replace(", ", "<br>")), _href=URL(c="manager", f="suggested_recommenders", vars=dict(articleId=row.id))
        )
    else:
...
def mkRecommendationFormat2(auth, db, row):
    ...
    anchor = DIV(artRep, BR(), row.recommendation_title, BR(), B(current.T("Recommender:") + " "), recommFmt, BR(), mkDOI(row.doi), _class="pci-RecommendationFormat2")
    return anchor
...
def mkRecommendationFormat(auth, db, row):
    ...
    if recommender:
        ...
        recommFmt = ""
    art = db.t_articles[row.article_id]
    anchor = SPAN(row.recommendation_title, BR(), B(current.T("Recommender:") + " "), recommFmt, BR(), mkDOI(row.doi),)
    return anchor
...
def mkRecommendation4PressReviewFormat(auth, db, row):
    ...
    anchor = DIV(
        SPAN(mkUserWithMail(auth, db, recomm.recommender_id)),
        BR(),
        mkDOI(recomm.doi),
        BR(),
