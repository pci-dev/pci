...
from py4web.core import URL as pydal_url
from yatl import DIV, P, SCRIPT
...
def URL(a: Optional[str] = None,
    ...
    return str(pydal_url(a, # type: ignore
                    c,
                    f,
                    r,
                    args,
...
def CENTER(*args: ..., **kwargs: ...):
    if "_style" in kwargs:
        kwargs["_style"] += "; text-align: center;"
    else:
        kwargs["_style"] = "text-align: center";
...
def get_next():
    if '_next' in request.query:
        next: list[str] | str = request.query.get('_next')
